...
public:
  ...
  void addPclAndKfPoseToQueue(const Eigen::MatrixXf &kfPcl,
                              const Eigen::Matrix4f &T_Wkf) {
    ...
    if (mIntegrating) {
      ...
      I3D_LOG(i3d::debug) << "addPclAndKfPoseToQueue" << kfPcl.cols() << "x"
                          << kfPcl.rows();
    }
  }
  ...
  void saveModel() {
    ...
    for (size_t i = 0; i < pclKfHost.size(); ++i) {
      ...
      for (int c = 0; c < ref3dList.cols(); ++c) {
        ...
        pclFile << pt[0] << " " << pt[1] << " " << pt[2] << " "
                << pt[4] * 255.0f << " " << pt[5] * 255.0f << " "
                << pt[6] * 255.0f << std::endl;
      }
    }
    ...
    //        property int vertex1                  { index to first vertex of
    //        edge } property int vertex2                  { index to second
    //        vertex } property uchar red                    { start of edge
    //        color } property uchar green property uchar blue kfFile <<
    //        "end_header\n"
    const float w = 0.1;
    const float h = w * 0.75;
    const float z = w * 0.6;
    const Eigen::Vector3f corner1(w, h, z), corner2(w, -h, z),
        corner3(-w, -h, z), corner4(-w, h, z);
    for (size_t nCurrCam = 0; nCurrCam < vpKfsF.size(); ++nCurrCam) {
      ...
      const Eigen::Vector3f pt1 = R * corner1 + T, pt2 = R * corner2 + T,
                            pt3 = R * corner3 + T, pt4 = R * corner4 + T;
      // camera center
      kfFile << camCenter[0] << " " << camCenter[1] << " " << camCenter[2]
             << " " << 0 << " " << 0 << " " << 255 << std::endl;
      // corner points
      kfFile << pt1[0] << " " << pt1[1] << " " << pt1[2] << " " << 0 << " " << 0
             << " " << 255 << std::endl;
      kfFile << pt2[0] << " " << pt2[1] << " " << pt2[2] << " " << 0 << " " << 0
             << " " << 255 << std::endl;
      kfFile << pt3[0] << " " << pt3[1] << " " << pt3[2] << " " << 0 << " " << 0
             << " " << 255 << std::endl;
      kfFile << pt4[0] << " " << pt4[1] << " " << pt4[2] << " " << 0 << " " << 0
             << " " << 255 << std::endl;
    }
    for (size_t nCurrCam = 0; nCurrCam < vpKfsF.size(); ++nCurrCam) {
      // draw the edges between the points. This should always be the same
      // order. (nCurrCam*5)+1,2,3,4.....
      const int ccIdx = nCurrCam * 5, pt1Idx = nCurrCam * 5 + 1,
                pt2Idx = nCurrCam * 5 + 2, pt3Idx = nCurrCam * 5 + 3,
                pt4Idx = nCurrCam * 5 + 4;
      kfFile << ccIdx << " " << pt1Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      kfFile << ccIdx << " " << pt2Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      kfFile << ccIdx << " " << pt3Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      kfFile << ccIdx << " " << pt4Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;

      kfFile << pt1Idx << " " << pt4Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      kfFile << pt1Idx << " " << pt2Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      kfFile << pt2Idx << " " << pt3Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      kfFile << pt3Idx << " " << pt4Idx << " " << 0 << " " << 0 << " " << 255
             << std::endl;
      if (nCurrCam > 0) // not the first camera
        kfFile << (nCurrCam - 1) * 5 << " " << ccIdx << " " << 0 << " " << 255
               << " " << 0 << std::endl;
    }
    kfFile.close();
    modelIsSaved = true;
