...
using namespace lsd_slam;
/**
 * This optimizer is a modified version of "LSD-SLAM: Large-scale direct
 * monocular SLAM, Engel et al., ECCV 2014"
 */
...
public:
  ...
  inline OptimizerSettings() {
    ...
    if (PYRAMID_LEVELS > 6)
      printf("WARNING: Sim3Tracker(): default settings are intended for a "
             "maximum of 6 levels!");

    // Levenberg-Marquardt parameters
    lambdaSuccessFac = 0.5f;
    lambdaFailFac = 2.0f;
    const float stepSizeMinc[6] = {1e-16f, 1e-16f, 1e-16f,
                                   1e-16f, 1e-16f, 1e-16f};
...
private:
  float calcErrorAndBuffers(const std::shared_ptr<ImgPyramidRGBD> &refFrame,
                            const std::shared_ptr<ImgPyramidRGBD> &currFrame,
                            const Eigen::Matrix3f &R, const Eigen::Vector3f &T,
                            ResidualInfo &resInfo, const uint lvl,
                            bool FILL_BUFFERS = true);
...
public:
  float
  bundleAdjustment(const std::vector<std::shared_ptr<ImgPyramidRGBD>> &frame,
                   Eigen::Matrix3f &R, Eigen::Vector3f &T, int lvl,
                   const std::shared_ptr<ImgPyramidRGBD> &currFrame);
  Optimizer(const OptimizerSettings &settings);
  ~Optimizer();
  float trackFrames(const std::shared_ptr<ImgPyramidRGBD> &refFrame,
                    const std::shared_ptr<ImgPyramidRGBD> &currFrame,
                    Eigen::Matrix3f &R, Eigen::Vector3f &T, int lvl,
                    ResidualInfo &resInfo);
  // float trackFrames(const std::shared_ptr<ImgPyramidRGBD> &refFrame, const
  // std::shared_ptr<ImgPyramidRGBD> &currFrame,
  //                             Eigen::Matrix3d &R, Eigen::Vector3d &T, int
  //                             lvl, ResidualInfo& resInfo);
  // Bilinear interpolation
  inline Eigen::Vector3f
  getInterpolatedElement43(const Eigen::Vector4f *const mat, const float x,
                           const float y, const int width) const {
