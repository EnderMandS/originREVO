          ...
          mSettings.settingsPyr)) //,windowedOptimization(mSettings.settingsWO)
{
  ...
  if (!mSettings.settingsIO.isFinished) {
    ...
    if (mSettings.DO_USE_PANGOLIN_VIEWER) {
      ...
      mpMapDrawer =
          std::shared_ptr<REVOGui::MapDrawer>(new REVOGui::MapDrawer());
      mpViewer =
    ...
    I3D_LOG(i3d::detail) << " camPyr.size():" << camPyr->size()
                         << " camPyr.area(0):" << camPyr->at(0).area;
...
void REVO::writePose(const Eigen::Matrix3f &R, const Eigen::Vector3f &T,
                     const double timeStamp) {
  const Eigen::Quaternionf Qf(R);
  mPoseFile << std::fixed << timeStamp << " " << std::setprecision(9) << T[0]
            << " " << T[1] << " " << T[2] << " " << Qf.x() << " " << Qf.y()
            << " " << Qf.z() << " " << Qf.w() << std::endl;
}
...
bool REVO::start() {
  ...
  if (mThIOWrapper == NULL) {
    mThIOWrapper = std::unique_ptr<std::thread>(
        new std::thread(&IOWrapperRGBD::generateImgPyramid, mIOWrapper));
  }
...
#endif
  {
    ...
    if (noFrames == 0) // first frame -> keyframe
    {
      ...
      if (mSettings.DO_OUTPUT_POSES)
        writePose(startPose.block<3, 3>(0, 0), startPose.block<3, 1>(0, 3),
                  tumRefTimestamp);
#ifdef WITH_PANGOLIN_VIEWER
      if (mSettings.DO_USE_PANGOLIN_VIEWER) {
        Eigen::MatrixXf pcl;
    ...
    if (trackerStatus == TrackerNew::TRACKER_STATE_NEW_KF &&
      ...
      if (mSettings.DO_USE_PANGOLIN_VIEWER) {
        ...
        mpMapDrawer->addPclAndKfPoseToQueue(
            pcl,
            kfPyr
                ->getTransKFtoWorld()); // mKeyframes.back()->getTransKFtoWorld());
    ...
    mTracker->addOldPclAndPose(currPyr->return3DEdges(mTracker->histogramLevel),
                               currPoseInWorld, currPyr->returnTimestamp());
    // Description:
    ...
    Eigen::Matrix4f absPose =
        mPoseGraph.back().getCurrToWorld(); // startPose*currPoseInWorld;
    I3D_LOG(i3d::debug) << "MY abs pose: "
                        << poseToTUMString(absPose.cast<double>(),
                                           tumRefTimestamp);
    // I3D_LOG(i3d::debug) << "TUM abs pose: "<<
    // poseToTUMString(mTumReader->getAbsPose(tumRefTimestamp),tumRefTimestamp);
    if (mSettings.DO_OUTPUT_POSES)
      writePose(absPose.block<3, 3>(0, 0), absPose.block<3, 1>(0, 3),
                tumRefTimestamp);
#ifdef WITH_PANGOLIN_VIEWER
    if (mSettings.DO_USE_PANGOLIN_VIEWER) {
  ...
  I3D_LOG(i3d::info) << "Distance Transform: "
                     << meandT / ImgPyramidRGBD::dtTimes.size();
