 ...
 *
 *
 * This optimizer is a modified version of LSD-SLAM's SE3 Tracker
 * http://vision.in.tum.de/lsdslam "LSD-SLAM: Large-scale direct monocular SLAM,
...
//{
//    //return;
//    I3D_LOG(i3d::info) << std::fixed << txt << ": " << t1[0] <<", "<< t1[1]<<
//    ", " << t1[2] << ", " << t1[3];
//}
void reprojectRefEdgesToCurrentFrameop(const cv::Mat &rgbCurr,
                                       const Eigen::MatrixXf &_3d,
                                       const Eigen::Matrix3f &K,
                                       const Eigen::Matrix3f &R,
                                       const Eigen::Vector3f &t,
                                       const cv::Mat &edgesCurr,
...
Optimizer::Optimizer(const OptimizerSettings &settings) : mSettings(settings) {
  ...
  buf_warped_residual =
      static_cast<float *>(Eigen::internal::aligned_malloc(memSize));
  buf_warped_dx =
      static_cast<float *>(Eigen::internal::aligned_malloc(memSize));
  buf_warped_dy =
      static_cast<float *>(Eigen::internal::aligned_malloc(memSize));
...
float Optimizer::calcErrorAndBuffers(
    const std::shared_ptr<ImgPyramidRGBD> &refFrame,
  ...
  const Eigen::Vector4f *distGradients =
  ...
  for (int c = 0; c < currPcl.cols(); ++c) {
    ...
    const Eigen::Vector3f resInterp =
        getInterpolatedElement43(distGradients, u_new, v_new, w);
    const float residual = resInterp[2];
    if ((residual > mSettings.edgeDistanceLvl[lvl]) &&
        mSettings.USE_EDGE_FILTER) {
      resInfo.badPtsEdges++;
      continue;
    }
  ...
  //        for (int c = 0; c < currPcl.cols();++c)
  //        {
  //            const Eigen::Vector4f refPoint = currPcl.col(c); //first three
  //            are the vector
  //            //IDEA: Use far points only for rotation!
  //            const Eigen::Vector3f Wxp = R * refPoint.head<3>() + T;
  //            const float u_new = Wxp[0]/Wxp[2]*cam.fx + cam.cx;
  ...
  //            {
  //                resInfo.badPtsEdges++;
  //                I3D_LOG(i3d::detail) << std::fixed << "Filter out of bounds:
  //                " << u_new << " " << v_new << " badPtsEdges: " <<
  //                resInfo.badPtsEdges; continue;
  //            }
  //            const Eigen::Vector3f resInterp =
  //            getInterpolatedElement43(distGradients, u_new, v_new, w); const
  //            float residual = resInterp[2]; if ((residual >
  //            mSettings.edgeDistanceLvl[lvl]) && mSettings.USE_EDGE_FILTER)
  //            {
  //                resInfo.badPtsEdges++;
  //                continue;
  //            }
  ...
  //           // I3D_LOG(i3d::info) << "residual: "<<residual << " resInfo: "
  //           << resInfo.sumErrorWeighted << " " << w_r;
  //            if (std::isnan(residual) || residual > 500.0f ||
  //            std::isinf(residual))
  //            {
  //                I3D_LOG(i3d::error) << residual << " " << Wxp.transpose() <<
  //                " " << u_new << " " << v_new << " " << cam.width << " " <<
  //                cam.height; I3D_LOG(i3d::info) <<
  //                refFrame->returnDistTransform(lvl)(cv::Rect(u_new-5,v_new-5,7,7));
  //                getInterpolatedElement43Debug(distGradients, u_new, v_new,
  //                w);
  //                //Output the last rows
  //                const Eigen::Vector4f* bp = distGradients;//
  //                +int(u_new)+int(v_new)*cam.width;
  //                //for (int i = int(u_new)+int(v_new)*cam.width; i <
  //                cam.area; ++i)
...
float Optimizer::trackFrames(const std::shared_ptr<ImgPyramidRGBD> &refFrame,
  ...
  for (int iteration = 0; iteration < mSettings.maxItsPerLvl[lvl];
    ...
    while (true) {
      ...
      Vector6 b = -ls.b;
      Matrix6x6 A = ls.A;
      // I3D_LOG(i3d::info)  << "Before lambda A: " <<A << "b: "<<b.transpose()
      // << LM_lambda;
      for (int i = 0; i < 6; i++)
        A(i, i) *= 1 + LM_lambda;
      ...
      // test.
      // Sophus::SE3d new_referenceToFrame =
      // Sophus::SE3d::exp(inc.cast<double>()) * referenceToFrame;
      Sophus::SE3f new_referenceToFrame =
          Sophus::SE3f::exp(inc) * referenceToFrame;
      // re-evaluate residual
      float error = calcErrorAndBuffers(
          refFrame, currFrame,
      ...
      if (error < lastErr) {
        ...
        if (error / lastErr > mSettings.convergenceEps[lvl]) {
          I3D_LOG(i3d::debug)
              << "(" << lvl << ", " << iteration << "," << error / lastErr
              << " ): FINISHED pyramid level (last residual reduction too "
                 "small).";
...
// float Optimizer::trackFrames(const std::shared_ptr<ImgPyramidRGBD> &refFrame,
// const std::shared_ptr<ImgPyramidRGBD> &currFrame,
//                             Eigen::Matrix3d &R, Eigen::Vector3d &T, int lvl,
//                             ResidualInfo& resInfo)
//{
...
//    lsd_slam::LGS6 ls;
//    float lastErr =
//    calcErrorAndBuffers(refFrame,currFrame,R.cast<float>(),T.cast<float>(),resInfo,lvl);
//    //exit(0);
//    float last_residual = lastErr;
//    float LM_lambda = mSettings.lambdaInitial[lvl];
...
//            Vector6 b = -ls.b;
//            Matrix6x6 A = ls.A;
//            //I3D_LOG(i3d::info)  << "Before lambda A: " <<A << "b:
//            "<<b.transpose() << LM_lambda; for(int i=0;i<6;i++) A(i,i) *=
//            1+LM_lambda; Vector6 inc = A.ldlt().solve(b); incTry++;
//            // apply increment. pretty sure this way round is correct, but
//            hard to test. Sophus::SE3d new_referenceToFrame =
//            Sophus::SE3d::exp(inc.cast<double>()) * referenceToFrame;
//            // re-evaluate residual
//            float error =
//            calcErrorAndBuffers(refFrame,currFrame,new_referenceToFrame.rotationMatrix().cast<float>(),
//                                              new_referenceToFrame.translation().cast<float>(),resInfo,lvl);
//            I3D_LOG(i3d::detail) << "After calcResidualAndBuffersEdges: " <<
//            error; I3D_LOG(i3d::info) <<"goodPts: " << resInfo.goodPtsEdges <<
//            " bad: " << resInfo.badPtsEdges << " tot: " <<
//            resInfo.goodPtsEdges+resInfo.badPtsEdges<<" error = "<< error
//            <<"=" << resInfo.sumErrorWeighted << "/" << resInfo.goodPtsEdges;
//            // accept inc?
//            if(error < lastErr)
//            {
//                // accept inc
...
//                if(error / lastErr > mSettings.convergenceEps[lvl])
//                {
//                    I3D_LOG(i3d::debug) << "(" << lvl <<", "<< iteration<< ","
//                    << error / lastErr <<" ): FINISHED pyramid level (last
//                    residual reduction too small)."; iteration =
//                    mSettings.maxItsPerLvl[lvl];
//                    //final_uncertainty = uncertainty;
//                }
//                last_residual = lastErr = error;
//                if(LM_lambda <= 0.2)
...
//                else
//                    LM_lambda *= mSettings.lambdaSuccessFac;
//                //I3D_LOG(i3d::info) << "Increment accepted: " <<
//                inc.transpose(); break;
//            }
//            else
//            {
...
//                {
//                    I3D_LOG(i3d::debug) << "(" << lvl <<", "<< iteration<<"):
//                    FINISHED pyramid level (stepsize too small)."; iteration =
//                    mSettings.maxItsPerLvl[lvl]; break;
//                }
//                if(LM_lambda == 0)
//                    LM_lambda = 0.2;
//                else
