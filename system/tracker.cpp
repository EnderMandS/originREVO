...
void TrackerNew::reprojectRefEdgesToCurrentFrame(
    const cv::Mat &rgbCurr, const Eigen::MatrixXf &_3d,
    const Eigen::Matrix3f &K, const Eigen::Matrix3f &R,
    const Eigen::Vector3f &t, const cv::Mat &edgesCurr,
  ...
  for (int ir = 0; ir < _3d.cols(); ir++) {
    ...
    if (res[0] >= 0 && res[0] < rgbCurr.cols && res[1] >= 0 &&
        res[1] < rgbCurr.rows) {
...
cv::Mat reprojectRefEdgesToDistTransform(
    const cv::Mat &dtC, const Eigen::MatrixXf &_3d, const Eigen::Matrix3f &K,
    const Eigen::Matrix3f &R, const Eigen::Vector3f &t,
    const cv::Mat &edgesCurr, const std::string title = "",
...
//
/**
 * @brief TrackerNew::assessTrackingQuality We project old pcls in the current
 * frame using the estimated pose. If the quality is not good enough, we take
 * the previous one as keyframe and re-estimate. See Schenk and Fraundorfer,
 * IROS 2017 paper, for a more detailed description
 * @param estimatedPose
 * @param mCurrFrame
 * @return
 */
TrackerNew::TrackerStatus TrackerNew::assessTrackingQuality(
    const Eigen::Matrix4f &estimatedPose,
    const std::shared_ptr<ImgPyramidRGBD> &mCurrFrame) {
  if (mPastPcl.size() != mPastWorldPoses.size() || mPastPcl.size() == 0 ||
      !mSettings.CHECK_TRACKING_RESULTS)
  ...
  for (int frame = 0; frame < mSettings.nFramesHistogramVoting &&
    ...
    const Eigen::Matrix4f transform =
        estimatedPose.inverse() *
        mPastWorldPoses.at(
            frame); // mPastWorldPoses.at(frame).inverse()*estimatedPose;
    ...
    for (size_t ir = 0; ir < pclSize; ir++) {
      ...
      if (newPt[0] >= 0 && newPt[0] < cam.width && newPt[1] >= 0 &&
          newPt[1] < cam.height)
        M_i.at<uint8_t>(floor(newPt[1]), floor(newPt[0])) =
            1; // prevent coinciding reprojections
      else
        ++outOfBounds;
    }
    cv::add(M_i, M, M);
  ...
  for (int xx = 0; xx < currEdges.cols; ++xx)
    for (int yy = 0; yy < currEdges.rows; ++yy) {
      ...
      if (mCurrFrame->isPointOkDepth(Z)) {
        ...
        if (currEdges.at<uint8_t>(yy, xx) > 0)
          overlaps.at(val) = overlaps.at(val) + 1;
      }
    }
          ...
          4) // || overlaps.at(histogram.size()-1) > overlaps.at(1)*0.5)
  {
    ...
    cv::imwrite("out/currEdges" + std::to_string(mCurrFrame->frameId) +
                    "_no.png",
                currEdges);
    cv::imwrite("out/currDepth" + std::to_string(mCurrFrame->frameId) +
                    "_no.png",
                currDepth);
...
/**
 * @brief TrackerNew::addOldPclAndPose Addes old pcl to list for tracking
 * quality assessment
 * @param pcl point cloud to be added
 * @param worldPose
 * @param timeStamp
 */
void TrackerNew::addOldPclAndPose(const Eigen::MatrixXf &pcl,
                                  const Eigen::Matrix4f &worldPose,
...
TrackerNew::TrackerNew(const TrackerSettings &config,
                       const ImgPyramidSettings &pyrConfig)
...
/**
 * @brief TrackerNew::clearUpPastLists clears the pcl lists used for tracking
 * quality assessment
 */
...
/**
 * @brief TrackerNew::checkInitializationValues Evaluates the cost function at
 * at mRefFrame using the 3D PCl from mCurrFrame R * PCL_curr * T
 ...
 */
void TrackerNew::checkInitializationValues(
    Eigen::Matrix3f &R, Eigen::Vector3f &T,
    const std::shared_ptr<ImgPyramidRGBD> &mCurrFrame,
    const std::shared_ptr<ImgPyramidRGBD> &mRefFrame) const {
  if (!mSettings.CHECK_INIT_VALUES)
    return;
...
/**
 * @brief TrackerNew::trackFrames Estimates the rotation R and translation T
 * from currFrame to refFrame
...
TrackerNew::trackFrames(Eigen::Matrix3f &R, Eigen::Vector3f &T, float &error,
  ...
  if (mSettings.optimizerSettings.DO_SHOW_DEBUG_IMAGES) {
    ...
    reprojectRefEdgesToCurrentFrame(
        refFrame->returnGray(0), currFrame->return3DEdges(0),
        currFrame->returnK(0), R, T, refFrame->returnEdges(0), "init");
    reprojectRefEdgesToDistTransform(
        refFrame->returnDistTransform(0), currFrame->return3DEdges(0),
  ...
  for (int lvl = mPyrConfig.PYR_MIN_LVL; lvl >= mPyrConfig.PYR_MAX_LVL; --lvl) {
    ...
    // I3D_LOG(i3d::info) << "Building acc structure at lvl: "<<lvl<<": " <<
    // Timer::getTimeDiffMs(startAcc,endAcc);
...
float TrackerNew::evalCostFunction(
    const Eigen::Matrix3f &R, const Eigen::Vector3f &T, uint minLvl,
    const std::shared_ptr<ImgPyramidRGBD> &mCurrFrame,
    const std::shared_ptr<ImgPyramidRGBD> &mRefFrame) const {
  ...
  for (int ir = 0; ir < _3d.cols(); ir++) {
    ...
    if (newPt[0] >= 0 && newPt[0] < size.width && newPt[1] >= 0 &&
      ...
      const float residual =
          distanceTransform.at<float>(floor(newPt[1]), floor(newPt[0]));
      if (residual > mSettings.optimizerSettings.edgeDistanceLvl[minLvl] &&
          mSettings.optimizerSettings.USE_EDGE_FILTER) {
        badCount++;
        continue;
      }
      goodCount++;
