...
public:
  TrackerSettings(
      const std::string &settingsFile) // distThreshold(0.1),angleThreshold(5),distThresholdTrackingLost(0.5),
                                       // angleThresholdTrackingLost(15)
  {
...
public:
  ...
  enum TrackerStatus {
    TRACKER_STATE_OK,
    TRACKER_STATE_LOST, // this should switch to relocalization, which is
  ...
  TrackerStatus trackFrames(Eigen::Matrix3f &R, Eigen::Vector3f &T,
                            float &error,
                            const std::shared_ptr<ImgPyramidRGBD> &refFrame,
                            const std::shared_ptr<ImgPyramidRGBD> &currFrame);
  TrackerNew(const TrackerSettings &config,
  ...
  TrackerStatus
  assessTrackingQuality(const Eigen::Matrix4f &estimatedPose,
                        const std::shared_ptr<ImgPyramidRGBD> &mCurrFrame);

  // stores the point cloud for tracking quality assessment
  void addOldPclAndPose(const Eigen::MatrixXf &pcl,
                        const Eigen::Matrix4f &worldPose,
                        const double timeStamp);
  // const void returnColoredPcl(Eigen::MatrixXf& clrPcl, const Eigen::Matrix4f
  // globPose) const;
private:
  float
  evalCostFunction(const Eigen::Matrix3f &R, const Eigen::Vector3f &T,
                   uint minLvl,
                   const std::shared_ptr<ImgPyramidRGBD> &mCurrFrame,
                   const std::shared_ptr<ImgPyramidRGBD> &mRefFrame) const;
  void checkInitializationValues(
  ...
  ////that would mean, we only have transform them with the inverse
  ///transformation!
  ...
  // debugging methods
  void reprojectRefEdgesToCurrentFrame(const cv::Mat &rgbCurr,
