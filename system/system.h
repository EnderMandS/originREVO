...
public:
  REVOConfig(const std::string &settingsFile, const std::string &dataSettings,
             int nRuns = 0)
      : settingsPyr(dataSettings), settingsIO(dataSettings, nRuns),
...
public:
  ...
  public:
    ...
    inline const Eigen::Matrix4f T_W_N() const { return getCurrToWorld(); }
...
private:
  ...
  void writePose(const Eigen::Matrix3f &R, const Eigen::Vector3f &T,
                 const double timeStamp);
  // WindowedOptimization windowedOptimization;
  static inline std::string RTToTUMString(const Eigen::Matrix3d &R,
  ...
  static inline std::string poseToTUMString(const Eigen::Matrix4d &pose,
                                            const double timeStamp) {
