...
#if defined(I3D_LOGGING_ENABLE_LEVELNAMES) &&                                  \
    !defined(I3D_LOGGING_ENABLE_FILEINFO)
...
const char *const Logging::log_colors_[] = {"",
                                            I3D_LOGGING_COLOR_FATAL,
                                            I3D_LOGGING_COLOR_ERROR,
                                            I3D_LOGGING_COLOR_WARNING,
                                            I3D_LOGGING_COLOR_INFO,
                                            I3D_LOGGING_COLOR_DEBUG,
                                            I3D_LOGGING_COLOR_TRACE,
                                            I3D_LOGGING_COLOR_DETAIL};
...
Logging::Logging(LogLevel level, const std::string &file, int line,
                 const std::string &function, bool append_crlf)
    : append_crlf_(append_crlf) {
  // Note: The code using log_colors_ and level_indicators_ is efficient but
  // unsafe if level is "abused" or changes in LogLevel are not considered here!
  // Maybe add an assertion to increase code safety.
  ...
  if (enable_timestamp_) {
    ...
    stream_ << local_time->tm_hour << ":" << local_time->tm_min << ":"
            << local_time->tm_sec << I3D_LOGGING_SEPARATOR_TIME;

    stream_.width(min_width);
#else
    stream_ << std::setw(2) << local_time->tm_hour << ":" << std::setw(2)
            << local_time->tm_min << ":" << std::setw(2) << local_time->tm_sec
            << I3D_LOGGING_SEPARATOR_TIME;
#endif
