...
ImgPyramidRGBD::ImgPyramidRGBD(const ImgPyramidSettings &settings,
                               const std::shared_ptr<CameraPyr> &cameraPyr)
...
ImgPyramidRGBD::ImgPyramidRGBD(const ImgPyramidSettings &settings,
                               const std::shared_ptr<CameraPyr> &cameraPyr,
                               const cv::Mat &fullResRgb,
                               const cv::Mat &fullResDepth,
  ...
  // 640x480
  distPatchSizes.push_back(20);
  distPatchSizes.push_back(10);
  distPatchSizes.push_back(5);
...
void ImgPyramidRGBD::getEdges(cv::Mat &edges, const cv::Mat &gray) {
  I3D_LOG(i3d::detail) << mSettings.cannyThreshold1 << " "
                       << mSettings.cannyThreshold2;

  if (mSettings.DO_GAUSSIAN_SMOOTHING_BEFORE_CANNY) {
    cv::Mat smoothImg;
    cv::GaussianBlur(gray, smoothImg, cv::Size(7, 7), 2);
    cv::Canny(smoothImg, edges, mSettings.cannyThreshold1,
              mSettings.cannyThreshold2, 3, true); //...edges,150,100,...
  } else
    cv::Canny(gray, edges, mSettings.cannyThreshold1, mSettings.cannyThreshold2,
              3, true); //...edges,150,100,...
}

void ImgPyramidRGBD::fillInEdges(size_t lvl) {
  I3D_LOG(i3d::detail) << "fillInEdges(int lvl): " << lvl << " "
                       << histPyr.size() << " " << edgesPyr.size();
  ...
  // I3D_LOG(i3d::info) << "edges: " << edgesMod.size() << "PATCH_SIZE: " <<
  // PATCH_SIZE << " LOW: "<<PATCH_SIZE_LOW << dist; cv::imshow("edgesTop: ",
  // topEdges);
...
float ImgPyramidRGBD::generateDistHistogram(const cv::Mat &edges,
                                            int PATCH_SIZE) {
  cv::Mat dist = cv::Mat(edges.rows / PATCH_SIZE, edges.cols / PATCH_SIZE,
                         CV_8UC1, cv::Scalar(0));
  I3D_LOG(i3d::detail) << "edges: " << edges.size()
                       << "PATCH_SIZE: " << PATCH_SIZE << dist.size();
  for (int yy = 0; yy < edges.rows; ++yy)
    for (int xx = 0; xx < edges.cols; ++xx) {
      if (edges.at<u_int8_t>(yy, xx) > 0)
        dist.at<u_int8_t>(
            floor((float)yy / PATCH_SIZE),
            floor(
                (float)xx /
                PATCH_SIZE))++; //=
  ...
  // I3D_LOG(i3d::info) << "edges: " << edges.size() << " " << nEdges << "
  // percent: " << double(nEdges)/edges.total() << "dist: " <<  nDist << "=>
  // percent " << double(nDist)/(32.0f*24.0f); cv::imshow("topEdges",topEdges);
...
void ImgPyramidRGBD::addLevelEdge(const cv::Mat &gray, const cv::Mat &depth,
                                  const Camera &cam) {
  clock_t start = clock();
  ...
  cv::Mat edges;
  const size_t lvl =
      static_cast<size_t>(mSettings.PYR_MAX_LVL) + edgesPyr.size();
  cv::Canny(gray, edges, mSettings.cannyThreshold1, mSettings.cannyThreshold2,
            3, true);
  ...
  for (int xx = 0; xx < edges.cols; xx++) {
    for (int yy = 0; yy < edges.rows; yy++) {
      ...
      if (std::isfinite(Z) && Z > mSettings.DEPTH_MIN &&
          Z < mSettings.DEPTH_MAX) {
        ...
        if (edgePx > 0) {
          ...
          linIdx++;
          if (linIdx < 50) {
            I3D_LOG(i3d::detail) << std::fixed << linIdx << ": "
                                 << ref3dList.col(linIdx - 1).transpose();
          }
        }
      }
    }
  }
  I3D_LOG(i3d::trace) << "Lvl: " << lvl
                      << ". nEdges: " << linIdx; // << modDepth;
...
void ImgPyramidRGBD::makeKeyframe() {
  ...
  for (int lvl = 0; lvl < mSettings.nLevels(); ++lvl) {
    ...
    Eigen::Vector4f *optStruc =
        (Eigen::Vector4f *)Eigen::internal::aligned_malloc(
            int(cam.area) * sizeof(Eigen::Vector4f));
...
void ImgPyramidRGBD::buildOptimizationStructure(
    const cv::Mat &floatImg, Eigen::Vector4f *startPt) const {
...
void ImgPyramidRGBD::generateColoredPcl(uint lvl, Eigen::MatrixXf &clrPcl,
                                        bool densePcl) const {
  const Camera cam = cameraPyr->at(lvl);
    ...
    for (int yy = 0; yy < rgb.rows; yy++) {
      ...
      if (isPointOkDepth(Z)) {
        ...
        if (densePcl || edgePx > 0) // dense or valid edge
        {
          ...
          v << p3D[0], p3D[1], p3D[2], 1, clr[2] / 255.0f, clr[1] / 255.0f,
              clr[0] / 255.0f, 1;
          clrPcl.col(linIdx) = v;
          linIdx++;
        }
  ...
  I3D_LOG(i3d::detail) << clrPcl.cols() << " " << clrPcl.rows();
