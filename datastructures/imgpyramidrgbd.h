...
public:
  ...
  ImgPyramidRGBD();
  void addLevelEdge(const cv::Mat &gray, const cv::Mat &depth,
                    const Camera &cam);
  void getEdges(cv::Mat &edges, const cv::Mat &gray);
  // generates a dense or sparse (edges only) PCL in the respective camera
  // coordiante system
  void generateColoredPcl(uint lvl, Eigen::MatrixXf &clrPcl,
                          bool densePcl) const;

  ImgPyramidRGBD(const ImgPyramidSettings &mSettings,
                 const std::shared_ptr<CameraPyr> &cameraPyr);
  ImgPyramidRGBD(const ImgPyramidSettings &mSettings,
                 const std::shared_ptr<CameraPyr> &cameraPyr,
                 const cv::Mat &fullResRgb, const cv::Mat &fullResDepth,
  ...
  inline double returnTimestamp() const { return timeStamp; }
  inline uint returnMaxLvl() const { return this->mSettings.PYR_MAX_LVL; }
  inline uint returnMinLvl() const { return this->mSettings.PYR_MIN_LVL; }
...
public:
  ...
  const inline Eigen::Matrix4f getTransKFtoWorld() const { return T_w_f; }
  ...
  inline bool isPointOkDepth(const float depthVal) const {
    return (std::isfinite(depthVal) && depthVal > mSettings.DEPTH_MIN &&
            depthVal < mSettings.DEPTH_MAX);
  }

private:
  inline bool isPointOkEdgePyr(const float edgePixel,
                               const float depthVal) const {
    return (edgePixel > 0 && std::isfinite(depthVal) &&
            depthVal > mSettings.DEPTH_MIN && depthVal < mSettings.DEPTH_MAX);
  }
  void buildOptimizationStructure(const cv::Mat &floatImg,
                                  Eigen::Vector4f *startPt) const;
  ...
  // std::vector<std::shared_ptr<Eigen::Vector4f>,Eigen::aligned_allocator<Eigen::Vector4f>>
  ...
  // For depth, we need to deal with the special case, when depth is invalid (is
  // equal to 0). this was partly taken from InfiniTAM
  // (https://github.com/victorprad/InfiniTAM)
  void FilterSubsampleWithHoles(cv::Mat &out, const cv::Mat &in) const {
    if (!(out.rows == int(in.rows / 2) && out.cols == int(in.cols / 2)))
      return;
    ...
    for (int y = 0; y < out.rows; y++)
      for (int x = 0; x < out.cols; x++)
        filterSubsampleWithHoles(imageData_out, x, y, out.cols, imageData_in,
                                 in.cols);
  }
  inline void filterSubsampleWithHoles(float *imageData_out, const int x,
                                       const int y, const int newWidth,
                                       const float *imageData_in,
                                       const int origWidth) const {
